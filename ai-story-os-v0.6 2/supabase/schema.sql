...
-- v0.5 daily-choice + monetization funnel preparation
create table if not exists choice_chain_events (
  id uuid primary key default gen_random_uuid(),
  project_id uuid references projects(id) on delete cascade,
  episode_id uuid references episodes(id) on delete cascade,
  event_type text not null check(event_type in ('choice_shown','vote_result','result_applied','profile_visit','link_click','paid_conversion')),
  payload jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);
create index if not exists idx_choice_chain_project_created on choice_chain_events(project_id,created_at desc);
