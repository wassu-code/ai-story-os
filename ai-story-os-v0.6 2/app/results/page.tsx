...
export default function Results(){const[m,setM]=useState<M>(empty);const[d,setD]=useState<D|null>(null);const[saved,setSaved]=useState('');
 ...
 async function analyze(next：M=m){const j=await fetch('/api/monetization',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(next)}).then(r=>r.json());setD(j.data)}
