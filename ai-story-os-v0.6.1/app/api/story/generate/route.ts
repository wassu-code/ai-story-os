...
export async function POST(req: Request) {
  ...
  try {
    const response = await client.responses.create({
      ...
      input: [
        { role:'system', content:'You are the editorial AI for a Japanese interactive daily-life social series starring one consistent adult AI character. Generate exactly 3 simple operator-level post concepts. Each concept must become a viewer-facing A/B choice understandable in under one second. Use ordinary life only: clothes, hair, food, after-work plans, weekend plans, room, habits. Preserve yesterday\'s chosen result and create a choice chain so today can affect tomorrow. Avoid thriller, horror, random twists, sexual escalation, or melodrama. Optimize for participation, return viewing, and character attachment. Avoid repeating topics in avoidTopics. Use metrics only as evidence; never invent performance. Respect monetizationGate: during validation/retention do not add sales or profile CTA. If regenerate=true, all three concepts MUST be materially different from currentConcepts and avoidTopics: use different categories where possible, different viewer choices, and do not paraphrase the current three. Output Japanese.' },
        { role:'user', content: JSON.stringify({ episodeNumber, previousSummary, worldState:body.worldState || {}, metrics:body.metrics || {}, funnel:body.funnel || {}, avoidTopics:body.avoidTopics || [], currentConcepts:body.currentConcepts || [], monetizationGate:body.monetizationGate || {}, regenerate, regenerationSeed:body.regenerationSeed || null }) }
      ],
